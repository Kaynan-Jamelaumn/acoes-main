...
class StudentPageView(BaseView):
    def __init__(self, model=Student, param_name="id", serializer=StudentSerializer):
        super().__init__(model, param_name, serializer)
    def get(self, request: HttpRequest, pk: str = None, page: int = 0) -> Response:
        return super().get(request, None, False, None, page)
...
class StudentCoursePageView(BaseView):
    def __init__(self, model=StudentCourse, param_name="id", serializer=StudentCourseSerializer):
        super().__init__(model, param_name, serializer)
...
class StudentCourse_Status_View(BaseView):
    def __init__(self, model=Student, param_name="id", serializer=StudentSerializer):
        super().__init__(model, param_name, serializer)
        self.allowed_methods= ['post', 'put', 'delete']
        self.disability_mapping = {
            "Não PCD": "Nenhum",
            "Física": "Deficiência Física",
            "Auditiva/surdez": "Surdez",
    ...
    def get_institute_object(self, campus, campus_mapping):
        campus = campus_mapping.get(campus, campus)
        campus_regex = r'\b{}\b'.format(re.escape(campus.lower()))

        return Institute.objects.filter(name__iregex=campus_regex).first()
    def post(self, request: HttpRequest) -> Response:
        if 'post' not in self.allowed_methods:
            return Response({"error": "Method not allowed"}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
        if not self.is_allowed(request):
        ...
        if request.content_type == 'application/json':
            ...
            for row in items[15000:]:
                try:
                    ...
                    institute_object = self.get_institute_object(campus, self.campus_mapping)
                    ...
                    if students_with_same_details.exists():
                        student = students_with_same_details.first()
                    if (not student): return Response({'error': 'Student not found: {}'.format(str(e))}, status=status.HTTP_400_BAD_REQUEST)


                    if admission_process in self.admission_process_mapping:
                        admission_process = self.admission_process_mapping[admission_process]
                        student_course_objects = StudentCourse.objects.filter(
                            student=student,
                        ...
                        if student_course_objects.exists():
                            student_course_object = student_course_objects.first()
                        else:
                            try:
                                student_course_object = StudentCourse.objects.create(
                                    student=student,
                                    course=course_object,
                                    ingressed_semester=ingressed_semester,
                                    admission_process=admission_process
